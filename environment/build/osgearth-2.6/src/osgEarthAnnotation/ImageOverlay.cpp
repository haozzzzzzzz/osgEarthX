...
void ImageOverlay::initMapNode( MapNode* pMapNode )
{
	AnnotationNode::initMapNode( pMapNode );
	postCTOR();
}
...
ImageOverlay::postCTOR()
{
	...
	if ( _drapeableNode == NULL )
	{
		// place the geometry under a drapeable node so it will project onto the terrain    
		_drapeableNode = new DrapeableNode( getMapNode() );
		addChild( _drapeableNode );

		_drapeableNode->addChild( _transform );
	}

	_drapeableNode->setMapNode( getMapNode() );
