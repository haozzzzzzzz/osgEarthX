...
MPTerrainEngineNode::postInitialize( const Map* map, const TerrainOptions& options )
{
    ...
    _liveTiles->setRevisioningEnabled( true );
...
MPTerrainEngineNode::refresh(bool forceDirty)
{
    ...
    else
    {
        ...
        else
        {
			...
			//$$$
			m_SingleThreadBeltline.addTaskAndWakeUp( new RefreshTerrainTask( this ) );
        }
...
MPTerrainEngineNode::createTerrain()
{
    ...
    unsigned child = 0;
	std::cout<<"MPTerrainEngineNode::createTerrain() ( @osgEarthDrivers\\..\\MPTerrainEngineNode.cpp ) : Begin create KeyNode"<<std::endl;//$$$
...
void MPTerrainEngineNode::_createNewTerrain( TilePagedLOD* pTileRoot )
{
	// scrub the heightfield cache.
	if (_tileModelFactory)
		_tileModelFactory->getHeightFieldCache()->clear();
	...
	_terrain->addChild( pTileRoot );
	_rootTilesRegistered = false;
	updateState();
}

RefreshTerrainTask::RefreshTerrainTask( MPTerrainEngineNode* pNode )
{
	m_pMPTerrainEngineNode = pNode;
}

void RefreshTerrainTask::doTask()
{
	m_pMPTerrainEngineNode->_createTerrainTilePagedLOD();
}

RefreshTerrainOperation::RefreshTerrainOperation(  MPTerrainEngineNode* pNode, TilePagedLOD* pTileRoot  )
{
	m_pMPTerrainEngineNode = pNode;
	m_pTilePagedLODRoot = pTileRoot;
}

void RefreshTerrainOperation::operator()( osg::Object* )
{
	m_pMPTerrainEngineNode->_createNewTerrain( m_pTilePagedLODRoot );
}