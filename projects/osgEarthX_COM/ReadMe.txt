========================================================================
    活动模板库 : osgEarthX_COM 项目概述
========================================================================

应用程序向导已为您创建了此 osgEarthX_COM 项目，作为编写动态链接库 (DLL) 
的起点。

本文件概要介绍组成项目的每个文件的内容。

osgEarthX_COM.vcxproj
    这是使用应用程序向导生成的 VC++ 项目的主项目文件，
    其中包含生成该文件的 Visual C++ 的版本信息，
    以及有关使用应用程序向导选择的平台、配置和项目功能的信息。

osgEarthX_COM.vcxproj.filters
...
osgEarthX_COM.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(osgEarthX_COM.h)
        GUID 定义			(osgEarthX_COM_i.c)
        类型库				(osgEarthX_COM.tlb)
        封送处理代码			(osgEarthX_COM_p.c 和 
                                           dlldata.c）

osgEarthX_COM.h
    此文件包含 osgEarthX_COM.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 osgEarthX_COM.pch 的预编译头 (PCH) 文件和
...
osgEarthX_COMps.vcxproj
    此文件是用于生成代理/存根 (stub) DLL 的项目文件（若有必要）。主项目中的 IDL 
    文件必须至少包含一个接口，并且在生成代理/存根 (stub) DLL 之前必须先编译 IDL 
    文件。此进程生成 dlldata.c、osgEarthX_COM_i.c 和 
    osgEarthX_COM_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

osgEarthX_COMps.vcxproj.filters
